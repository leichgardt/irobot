...
async def feedback_inline_h(query: types.CallbackQuery, state: FSMContext):
    await bot.logger.info(f'[{query.message.chat.id}] Feedback')
    await state.finish()
...
async def comment_feedback_inline_h(query: types.CallbackQuery, state: FSMContext):
    async with state.proxy() as data:
        await bot.logger.info(f'[{query.message.chat.id}] Feedback passed')
        await state.finish()
...
async def feedback_comment_message_h(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        await bot.logger.info(f'[{message.chat.id}] Feedback commented ')
        await state.finish()
