...
from src.bot.api import update_inline_query, private_and_login_require, exc_handler
...
async def inline_h_account_settings(query: types.CallbackQuery, state: FSMContext):
    async with state.proxy() as data:
        data['accounts'] = await agreements.get_all_agrm_data(query.message.chat.id, only_numbers=True)
